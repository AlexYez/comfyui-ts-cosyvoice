...
def split_cyrillic_paragraph(text: str, tokenize, token_max_n: int = 160, token_min_n: int = 120, merge_len: int = 32):
    sentences = [chunk.strip() for chunk in re.findall(r'[^.!?;:]+[.!?;:…]*', text) if chunk.strip()]
    if not sentences:
        cleaned = text.strip()
        return [cleaned] if cleaned else []

    def calc_token_length(value: str) -> int:
        return len(tokenize(value))

    final_utts = []
    cur_utt = ""
    for sentence in sentences:
        candidate = f"{cur_utt} {sentence}".strip() if cur_utt else sentence
        if cur_utt and calc_token_length(candidate) > token_max_n and calc_token_length(cur_utt) >= token_min_n:
    ...
    if cur_utt:
        if calc_token_length(cur_utt) < merge_len and final_utts:
            final_utts[-1] = f"{final_utts[-1]} {cur_utt}".strip()
        else:
            final_utts.append(cur_utt)
    return final_utts
    ...
    def text_normalize(self, text, split=True, text_frontend=True):
        ...
        if self.use_ttsfrd:
            ...
            if contains_chinese(text):
                ...
                text = text.replace("\n", " ").strip()
                texts = split_cyrillic_paragraph(
                    text,
